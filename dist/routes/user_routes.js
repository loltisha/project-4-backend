...
router.post("/sign-up", function (req, res, next) {
  ...
  Promise.resolve(req.body.credentials).then(function (credentials) {
    console.log('credentials: ' + credentials.password_confirmation);
    ...
    req.login(payload, { session: false }, function (error) {
      ...
      res.cookie("jwt", token, { httpOnly: true, secure: false }).status(201).json({ id: req.user.id, email: req.user.email, type: req.user.type });
    });
  })
...
router.post("/sign-in", localAuth, function (req, res, next) {
  if (req.user) {
    ...
    req.login(payload, { session: false }, function (error) {
      ...
      // assign our jwt to the cookie
      res.cookie("jwt", token, { httpOnly: true, secure: false }).status(200).json({ id: req.user.id, email: req.user.email, type: req.user.type });
    });
  }
});
